export async function defileGround({ speaker, actor, token, character, item, args, scope, workflow, options }) {
    ...
    if(args[0].macroPass === "postActiveEffects") {
        ...
        if (effectCheck) {
            ...
            let updateData = {
                _id: effectCheck.id,
                changes: effectCheck.changes
            };

            await MidiQOL.socket().executeAsGM("updateEffects", { actorUuid: actor.uuid, updates: [updateData] });
        }
    ...
    if (args[0] === "off") {
        ...
        if(!effectCheck)
        {
            ...
            const effectName = "Defile Ground";
