                        ...
                        <div class="gps-dialog-content">
                            <div>
                                <div class="gps-dialog-flex">
                                    <p class="gps-dialog-paragraph">${game.i18n.format("GAMBITSPREMADES.Dialogs.Automations.Spells.Counterspell.Prompts.UseYourReaction.Default", { itemName: itemProperName })}</p>
            ...
            if (!userDecision && isLastToken) {
                ...
                if(itemRollCastLevel < castLevel) {
                    ...
                    if (skillTotal >= spellThreshold) {
                        chatContent = `<span style='text-wrap: wrap;'>${game.i18n.format("GAMBITSPREMADES.ChatMessages.Automations.Spells.Counterspell.CounterspellSuccess", { skillTotal: skillTotal, skillFlavor: skillFlavor, abjurationChat: abjurationChat })}<br><img src="${selectedToken.actor.img}" width="30" height="30" style="border:0px"></span>`;
                        counterspellLevel = itemRollCastLevel;
                    }
                    else {
                        chatContent = `<span style='text-wrap: wrap;'>${game.i18n.format("GAMBITSPREMADES.ChatMessages.Automations.Spells.Counterspell.CounterspellFailed", { skillTotal: skillTotal, skillFlavor: skillFlavor, abjurationChat: abjurationChat, spellThreshold: spellThreshold, spellPenetrationChat: spellPenetrationChat })}<br><img src="${selectedToken.actor.img}" width="30" height="30" style="border:0px"></span>`;
                        csFailure = true;
                    }
                }
                else {
                    chatContent = `<span style='text-wrap: wrap;'>${game.i18n.format("GAMBITSPREMADES.ChatMessages.Automations.Spells.Counterspell.CounterspellAutoSuccess", { itemName: itemProperName })}<br><img src="${selectedToken.actor.img}" width="30" height="30" style="border:0px"></span>`;
                    counterspellLevel = itemRollCastLevel;
                }
    ...
    async function secondaryCounterspellProcess(workflow, lastMessage, castLevel, validTokenPrimary) {        
            ...
            if (!userDecision && isLastToken) {
                ...
                if(itemRollCastLevel < castLevel) {
                    ...
                    if (skillTotal >= spellThreshold) {
                        chatContent = `<span style='text-wrap: wrap;'>${game.i18n.format("GAMBITSPREMADES.ChatMessages.Automations.Spells.Counterspell.CounterspellSuccess", { skillTotal: skillTotal, skillFlavor: skillFlavor, abjurationChat: abjurationChat })}<br><img src="${validTokenPrimary.actor.img}" width="30" height="30" style="border:0px"></span>`;
                        counterspellLevel = itemRollCastLevel;
                    }
                    else {
                        chatContent = `<span style='text-wrap: wrap;'>${game.i18n.format("GAMBITSPREMADES.ChatMessages.Automations.Spells.Counterspell.CounterspellFailed", { skillTotal: skillTotal, skillFlavor: skillFlavor, abjurationChat: abjurationChat, spellThreshold: spellThreshold, spellPenetrationChat: spellPenetrationChat })}<br><img src="${validTokenPrimary.actor.img}" width="30" height="30" style="border:0px"></span>`;
                        csFailure = true;
                    }
                }
                else {
                    chatContent = `<span style='text-wrap: wrap;'>${game.i18n.format("GAMBITSPREMADES.ChatMessages.Automations.Spells.Counterspell.CounterspellAutoSuccess", { itemName: itemProperName })}<br><img src="${validTokenPrimary.actor.img}" width="30" height="30" style="border:0px"></span>`;
                    counterspellLevel = itemRollCastLevel;
                }
