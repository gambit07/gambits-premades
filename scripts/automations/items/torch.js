import {
    showLightDialog,
    showLightThrowDialog,
    showLightThrowOptionsDialog,
    showLightOptionsDialog,
    lightExtinguish,
    showThrowExtinguishDialog
  ...
  export async function torch({ speaker, actor, token, item, args, workflow }) {
    ...
    const activeLight = actor.appliedEffects.find(e => e.flags["gambits-premades"]?.gpsUuid === gpsUuid);
    ...
    if (!activeLight) {
      if (!hasRemaining) {
        ...
        await showLightThrowDialog({ token, item, effectName, filePath, lightEffect, animType, gpsUuid });
      } else {
        await showLightDialog({ token, item, effectName, filePath, lightEffect, animType, gpsUuid });
      }
    } else {
      if(!hasRemaining) {
        await showThrowExtinguishDialog({ token, item, effectName, filePath, lightEffect, animType, gpsUuid, actor });
      }
      else if (canThrow) {
        await showLightThrowOptionsDialog({ token, item, effectName, filePath, lightEffect, animType, gpsUuid, actor });
      } else {
        await showLightOptionsDialog({ token, item, effectName, filePath, lightEffect, animType, gpsUuid, actor });
      }
    }
  }